...
# CONFIGURATION
# ===============================
INPUT_DIR="/mnt/CEPH_PROJECTS/Environtwin/GDD/SouthTyrol/frost"
OUTPUT_DIR="/mnt/CEPH_PROJECTS/Environtwin/FORCE/masks/frost"
...
export PROCESSED_LOG SKIPPED_LOG ERROR_LOG_DIR
#MISSING_LOG 
# ===============================
# TOTAL IMAGES COUNT
# ===============================
TOTAL_IMAGES=$(find "$INPUT_DIR" -type f -name 'first_frost_*.tif' | wc -l)
export TOTAL_IMAGES
...
process_tiff() {
    ...
    if ! find "$OUTPUT_DIR" -type f -name "${BASENAME}*" -print -quit | grep -q .; then
        echo "$BASENAME" >> "$MISSING_LOG"
        echo "⚠️ No output found for $TIFF_FILE"
        return
    fi
...
# MAIN
# ===============================
find "$INPUT_DIR" -type f -name 'first_frost_*.tif' | \
