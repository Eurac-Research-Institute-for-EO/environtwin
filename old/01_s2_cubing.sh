...
OUTPUT_DIR="/mnt/CEPH_PROJECTS/Environtwin/FORCE/level2_sites_raw"
...
for DIR in "${INPUT_DIRS[@]}"; do
    COUNT=$(find "$DIR" -type f -name '*_SEN2?_BOA.tif' | wc -l)
...
process_tiff() {
   ...
   # Skip if output already exists, excluding some subfolders
   # EXCLUDE_FOLDERS=("X0006_Y0000" "X0006_Y0001" "X0007_Y0000" "X0007_Y00-1")
   # FIND_CMD=(find "$OUTPUT_DIR" -type f -name "${BASENAME}*")
    #for F in "${EXCLUDE_FOLDERS[@]}"; do
     #   FIND_CMD+=(-not -path "$OUTPUT_DIR/$F/*")
    #done

    #if "${FIND_CMD[@]}" | grep -q .; then
     #   echo "$BASENAME" >> "$SKIPPED_LOG"
      #  echo "Skipping $TIFF_FILE (output already exists outside excluded folders)"
       # return
    #fi
...
# ===============================
for DIR in "${INPUT_DIRS[@]}"; do
    #find "$DIR" -type f -name '*_SEN2?_BOA.tif' | \
