...
def find_pairs():
    ...
    for folder in input_dirs:
        ...
        boa_pattern = os.path.join(folder, "*_PLANET_*_BOA.tif")
        ...
        # Find UDM files matching date pattern DIRECTLY with glob
        udm_pattern = os.path.join(folder, "*_PLANET_udm2_mask.tif")
...
def process_date_subfolder(args):
    ...
    out_udm = os.path.join(out_dir, f"{date}_PLANET_udm2_mask.tif")
    ...
    with rasterio.open(ref_boa_fp) as ref:
        ...
        band_descriptions = ref.descriptions  # Preserve band names
        boa_profile = ref.profile.copy()
        #boa_profile.update(dtype="int16", nodata=-9999, compress="deflate")

    with rasterio.open(ref_udm_fp) as refm:
        udm_bands = refm.count
        udm_profile = refm.profile.copy()
        # Keep original nodata for consistency, but output as int16 to support -9999
        #udm_profile.update(dtype="int16", compress="deflate", nodata=-9999)

    cnt_profile = boa_profile.copy()
    cnt_profile.update(count=1, dtype="int16", nodata=-9999)

    # Allocate arrays
    sum_boa = np.zeros((bands, height, width), dtype=np.int16)
    cnt_boa = np.zeros((bands, height, width), dtype=np.int16)
    cnt_img = np.zeros((height, width), dtype=np.int16)  # Count of clear images
    sum_udm = np.zeros((udm_bands, height, width), dtype=np.int16)
    cnt_udm = np.zeros((height, width), dtype=np.int16)  # Count of images with UDM data
    ...
    for boa_fp, udm_fp in file_pairs:
        ...
        with rasterio.open(boa_fp) as boa, rasterio.open(udm_fp) as udm:
            ...
            for _, win in boa.block_windows(1):
                ...
                # Determine pixels with UDM data (UDM can have data even where BOA is nodata)
                ...
                for b in range(bands):
                    ...
                    cnt_boa[b, rr, cc] += valid_for_boa.astype(np.int16)

                # Count contributing images (only clear pixels)
                cnt_img[rr, cc] += valid_for_boa.astype(np.uint16)

                # Accumulate UDM for ALL pixels with UDM data (including clear=0 areas)
                # This preserves the full quality information including cloudy pixels
                for b in range(udm_bands):
                    sum_udm[b, rr, cc] += np.where(has_udm_data, udm_win[b], 0).astype(np.int16)

                # Track pixels with UDM data
                cnt_udm[rr, cc] += has_udm_data.astype(np.int16)
    ...
    for b in range(udm_bands):
        udm_final[b] = np.where(
            udm_has_data,
            (sum_udm[b] > 0).astype(np.int16),  # If any input had this flag, output is 1; else 0
            -9999  # Set to nodata for areas without coverage
        )
    ...
    if n == 1:
        ...
        with rasterio.open(ref_udm_fp) as src:
            ...
            with rasterio.open(out_udm, "w", **udm_profile) as dst:
                dst.write(data)
                # Preserve UDM band descriptions
        ...
        with rasterio.open(out_udm, "w", **udm_profile) as dst:
            dst.write(udm_final)
            # Set UDM band descriptions (read from reference)
            with rasterio.open(ref_udm_fp) as src:
                udm_descriptions = src.descriptions
                for i, desc in enumerate(udm_descriptions, start=1):
                    if desc:
    ...
    with rasterio.open(out_cnt, "w", **cnt_profile) as dst:
        dst.write(cnt_img, 1)
        dst.set_band_description(1, "count")  # Add description for count band
