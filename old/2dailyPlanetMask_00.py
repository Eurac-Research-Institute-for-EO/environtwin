...
def process_date_subfolder(args):
    ...
    cnt_img = np.zeros((height, width), dtype=np.uint16)  # Count of clear images
    sum_udm = np.zeros((udm_bands, height, width), dtype=np.uint16)
    cnt_udm = np.zeros((height, width), dtype=np.uint16)  # Count of images with UDM data
        ...
        with rasterio.open(boa_fp) as boa, rasterio.open(udm_fp) as udm:
            ...
            for _, win in boa.block_windows(1):
                ...
                for b in range(bands):
                    ...
                    cnt_boa[b, rr, cc] += valid_for_boa.astype(np.uint16)
