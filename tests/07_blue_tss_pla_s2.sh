#!/bin/bash
# ===============================================
# Purpose: Build annual temporal stacks of BLUE band (band 1)
#          from both PLANET and SENTINEL BOA files.
#          Keep only March–November scenes.
#          Output: ENVI BSQ with updated header
# ===============================================

INPUT_DIR="/mnt/CEPH_PROJECTS/Environtwin/FORCE/level2_sites_daily/01/MH"
INPUT_SEN2="/mnt/CEPH_PROJECTS/Environtwin/FORCE/level2_sites_raw/MH/SEN2"
BASE_OUTPUT="/mnt/CEPH_PROJECTS/Environtwin/FORCE/level3_sites/indices/"

# ================================
# MAIN LOOP FOR MULTIPLE TILES
# ================================
for i in "${!INPUT_DIR[@]}"; do
    INPUT_DIR="${INPUT_DIR[$i]}"
    SEN2_DIR="${INPUT_SEN2[$i]}"

    echo "=============================================="
    echo "📂 Processing PLANET folder: $INPUT_DIR"
    echo "📂 Corresponding Sentinel folder: $SEN2_DIR"
    echo "=============================================="
    ...
    # Collect all PLANET + SENTINEL NDVI files
    FILES_ALL=($(find "$INPUT_DIR" "$SEN2_DIR" -maxdepth 1 -type f \
        \( -name "*_PLANET_*_BOA.tif" -o -name "*_SEN2*_BOA.tif" \)))

    if [ ${#FILES_ALL[@]} -eq 0 ]; then
        echo "❌ No NDVI files found in $INPUT_DIR or $SEN2_DIR"
        continue
    fi
    ...
    for year in $YEARS; do
        ...
        # Collect files for that year (sorted chronologically)
        FILES=($(find "$INPUT_DIR" "$SEN2_DIR" -maxdepth 1 -type f \
    \( -name "${year}*_PLANET_*_BOA.tif" -o -name "${year}*_SEN2*_BOA.tif" \) \
    | awk -F'/' '{
        fname=$NF;
    ...
    | cut -d' ' -f2- ))
        ...
        BASE_NAME="${year}${START_DOY}-${year}${END_DOY}_TSA_SENPLA_BLU_TSS"

        OUTPUT_TXT="$OUTPUT_DIR/dates_${year}_SENPLA_BLU.txt"
        > "$OUTPUT_TXT"
        ...
        gdalbuildvrt -separate -b 1 -vrtnodata -9999 "$TEMP_VRT" "${FILES[@]}"

        OUTPUT_BSQ="$OUTPUT_DIR/${FIRST_DATE}-${LAST_DATE}_${START_DOY}-${END_DOY}_TSA_SENPLA_BLU_TSS.bsq"
