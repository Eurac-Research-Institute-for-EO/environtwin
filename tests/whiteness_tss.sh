#!/bin/bash
# ===============================================
# Purpose: Build annual Whiteness stacks from PLANET files
#          and export them as ENVI BSQ files with rich headers
# ===============================================


INPUT_DIRS=(
	"/mnt/CEPH_PROJECTS/Environtwin/FORCE/sites_haze"
	)

BASE_OUTPUT="/mnt/CEPH_PROJECTS/Environtwin/FORCE/sites_haze/indices"
...
for INPUT_DIR in "${INPUT_DIRS[@]}"; do
    ...
    echo "=============================================="
    echo "📂 Processing PLANET Whitness in tile: $INPUT_DIR"
    echo "=============================================="
    ...
    FILES_ALL=($(find "$INPUT_DIR" -maxdepth 1 -type f -name "*_PLA_*_BOA_haze.tif"))
    ...
    for year in $YEARS; do
        ...
        FILES=($(find "$INPUT_DIR" -maxdepth 1 -type f \
            -name "${year}*_PLA_*_BOA_haze.tif" | sort -V))
