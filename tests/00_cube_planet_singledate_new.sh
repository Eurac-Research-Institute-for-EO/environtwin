...
# CONFIGURATION
# ===============================
BASE_ROOT="/mnt/CEPH_PROJECTS/Environtwin/PLANET/PA"
OUTPUT_DIR="/mnt/CEPH_PROJECTS/Environtwin/FORCE/tmp"
...
for YEAR in "${YEARS[@]}"; do
    ...
    for ZIP_FILE in "$YEAR_DIR"/batch_*.zip; do
        ...
        TIFF_LIST=$(find "$EXTRACT_DIR/files" -type f -name '*3B_AnalyticMS_SR_harmonized_clip.tif')
        if [[ -z "$TIFF_LIST" ]]; then
