...
INPUT_DIRS=(
	"/mnt/CEPH_PROJECTS/Environtwin/FORCE/level2_sites_daily/02/MH"
	)

BASE_OUTPUT="/mnt/CEPH_PROJECTS/Environtwin/FORCE/level3_sites/indices/02"
...
for INPUT_DIR in "${INPUT_DIRS[@]}"; do
    ...
    echo "=============================================="
    echo "📂 Processing PLANET BLUE Band in tile: $INPUT_DIR"
    echo "=============================================="
    ...
    FILES_ALL=($(find "$INPUT_DIR" -maxdepth 1 -type f -name "*_PLANET_*_BOA.tif"))

    if [ ${#FILES_ALL[@]} -eq 0 ]; then
        echo "No PLANET files found in $INPUT_DIR"
    ...
    for year in $YEARS; do
        ...
        FILES=($(find "$INPUT_DIR" -maxdepth 1 -type f \
            -name "${year}*_PLANET_*_BOA.tif" | sort -V))

        if [ ${#FILES[@]} -eq 0 ]; then
            echo "No PLANET files for $year, skipping..."
            continue
        fi
        ...
        OUTPUT_TXT="$OUTPUT_DIR/dates_${year}_PLA_BLU.txt"
        > "$OUTPUT_TXT"
        ...
        OUTPUT_BSQ="$OUTPUT_DIR/${first_date}-${last_date}_${first_doy}-${last_doy}_TSA_PLA_BLU_TSS.bsq"
