...
for DIR in "${INPUT_DIRS[@]}"; do
    COUNT=$(find "$DIR" -type f -name '*_SEN2?.tif' | wc -l)
...
if [[ $# -eq 1 ]]; then
    ...
    FILE_PATH=$(find "${INPUT_DIRS[@]}" -type f -path "*/$SINGLE_FILE" | head -n 1)
	if [[ -z "$FILE_PATH" ]]; then
	    echo "❌ File not found in input directories: $SINGLE_FILE"
	    exit 1
	fi
...
# ===============================
for DIR in "${INPUT_DIRS[@]}"; do
    find "$DIR" -type f -name '*_SEN2?.tif' | \
...
rm "$PROCESSED_LOG" "$MISSING_LOG" "$SKIPPED_LOG"
